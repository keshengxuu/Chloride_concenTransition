classdef MeanFieldModel < RateNetwork
    ...
    properties (Constant)
        VarName = {'V','phi','Cl_in','g_K','E_Cl','w'}
    end
    ...
    methods % Simulation functions
        function IndividualModelUpdate(O,dt) 
            ...
            O.E_Cl = 26.7*log(O.Cl_in./O.param.Cl_ex); % Calculate E_Cl (inhibition effectiveness) 
            V_inf = (O.param.g_L .* O.param.E_L  + ...
                     O.Input.E ./ O.param.f_max .* O.param.E_Esyn + ...
                     O.Input.I ./ O.param.f_max .* O.E_Cl + ...  
            ...
            Faraday = 96500;               
            up = O.Input.I .* (O.V - O.E_Cl) ./ (O.param.Vd_Cl * Faraday);
            
            O.w = 0.94; % Wcon =0.94, generate spiral waves
            adaptive_up = up .* O.w;
            Cl_in_inf = adaptive_up .* O.param.tau_Cl + O.param.Cl_in_eq;
